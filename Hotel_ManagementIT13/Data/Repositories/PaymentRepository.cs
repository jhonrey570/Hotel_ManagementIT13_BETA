...
namespace Hotel_ManagementIT13.Data.Repositories
{
    public class PaymentRepository
    {
        ...
        public bool RefundPayment(int paymentId, string refundReason)
        {
            using (var conn = DatabaseHelper.GetConnection())
            {
                conn.Open();
                using (var transaction = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        string getPaymentQuery = @"
                            SELECT reservation_id, amount FROM payments 
                            WHERE payment_id = @paymentId";

                        int reservationId;
                        decimal amount;

                        using (var cmd = new MySqlCommand(getPaymentQuery, conn, transaction))
                        {
                        ...
                        string refundQuery = @"
                            ...
                            (reservation_id, payment_status_id, amount, payment_method, notes)
                            VALUES 
                            (@reservationId, 4, -@amount, 'Refund', @refundReason)";

                        using (var cmd = new MySqlCommand(refundQuery, conn, transaction))
                        {
                            cmd.Parameters.AddWithValue("@reservationId", reservationId);
                            cmd.Parameters.AddWithValue("@amount", amount);
                        ...
                        string updateQuery = @"
                            UPDATE payments SET payment_status_id = 4 
                            WHERE payment_id = @paymentId";
